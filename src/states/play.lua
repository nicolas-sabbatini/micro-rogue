...
function Play.wheelmoved(x, y)
	...
	if love.keyboard.isDown("lctrl") then
		offset_x, offset_y = UTILS.screen_to_hump(camera, FAKE_WIDTH, FAKE_HEIGHT)
	end
end
--
...
function Play.update(dt)
	...
	if mouse_y and mouse_y + 5 > FAKE_HEIGHT then
		offset_y = offset_y + (50 * dt / zoom)
	end
	if mouse_y and mouse_y - 5 < 0 then
		offset_y = offset_y + (-50 * dt / zoom)
	end
	if mouse_x and mouse_x + 5 > FAKE_WIDTH then
		offset_x = offset_x + (50 * dt / zoom)
	end
	if mouse_x and mouse_x - 5 < 0 then
		offset_x = offset_x + (-50 * dt / zoom)
	end
end
